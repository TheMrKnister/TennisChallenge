...
namespace TennisWeb.Controllers
{
  public class InfoBoardController : Controller
  {
    public ActionResult Index(Guid? clubId)
    {
      ...
      var courtAccesor = new AccessorBase<Court>();
      var courtModel = courtAccesor.GetAllWhere(c => c.ClubFk == clubId).Select(c => new CourtVm().From(c));
      ...
      foreach (var booking in jBookings)
      {
        ...
        switch (booking.court)
        {
          ...
          case "Platz 5":
            ...
            break;
          case "Platz 6":
           var court6DayDate = firstDayOfWeekInMonth.AddDays(28);
           court6DayDate = court6DayDate.AddDays(toAdd);
           var court6Day = Convert.ToInt32(court6DayDate.Day);
           model.Start = new DateTime(start.Year, court6DayDate.Month, court6Day, start.Hour, start.Minute, start.Second, start.Millisecond);
           model.End = new DateTime(end.Year, court6DayDate.Month, court6Day, end.Hour, end.Minute, end.Second, end.Millisecond);
           model.Row = "week4";
           break;
                }
