    ...
    public class ResourcesController : Controller
    {
        ...
        private IEnumerable<string> GetContentUrlsFromDirectory(string directoryName)
        {
            return Directory.GetFiles(
                Server.MapPath(string.Format("~/{0}", directoryName)), "*.*", SearchOption.AllDirectories)
                .Where(fileName => !fileName.EndsWith(".db"))
                .Select(fileName => Url.Content(
                    string.Format("~/{0}", fileName.Substring(fileName.IndexOf(directoryName, StringComparison.Ordinal)))
                    .Replace(@"\", "/")))
                .ToList();
        }
